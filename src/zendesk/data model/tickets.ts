...
import {ticketRatings}  from "./ticketSatisfactionRatings";
import {ticketComments}  from "./ticketComments";
import {ticketAudits}  from "./ticketAudits";
var utils = require("../../utils/utils");

export class tickets {
    private QContent: contentQueue;
    public DataObjectName: String = C_DATAOBJECTNAME;
    public ratings: ticketRatings;
    public comments: ticketComments;
    public audits: ticketAudits;
    constructor(QContent: contentQueue) {
        this.QContent = QContent;
        this.ratings = new ticketRatings(QContent);
        this.comments = new ticketComments(QContent);
        this.audits = new ticketAudits(QContent);
        ...
        return new Promise((resolve, reject) => {
            ...
            cloudElements.GetElementObjectPage(accountData.CEelementInstanceToken, C_DATAOBJECTNAME, currentPage).then((elementsReturned: any) => {
                if (!elementsReturned || !elementsReturned.length) {
                    ...
                    _this.transform(accountData, elementsReturned).then((args: any) => {
                        ...
                        _this.ratings.transform(accountData, ticketsSatisfactionRatingsToMap).then((finalTicketsSatisfactionRatingsToMap: any) => {
                                ...
                                _this.comments.transform(accountData, commentsToWrite).then((finalTicketsCommentsToMap) => {
                                    utils.SetObjectPointersByArrayAndField(accountData.identifier, finalTicketsToMap,finalTicketsCommentsToMap,"comments","id","ticket_id","id").then((finalTicketsToMap) => {
                                        ...
                                        _this.audits.getTicketsAudits(accountData, ticketsIdsArray).then((auditsToWrite) => {
                                            _this.audits.transform(accountData, auditsToWrite).then((audits: any) => {
    ...
    private createTicket(subject, description, submitter_id, assignee_id, tags) {
        var newTicket = {
            "subject": subject,
